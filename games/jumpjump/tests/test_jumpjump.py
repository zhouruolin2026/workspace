...
@pytest.fixture
def game_url(driver):
    return "file:///Users/njh/.qclaw/workspace/games/jumpjump/index.html"


class TestJumpJumpGame:
    def test_page_loads(self, driver, game_url):
        driver.get(game_url)
        time.sleep(0.3)
        assert "跳一跳" in driver.title
    ...
    def test_start_and_pause_state(self, driver, game_url):
        driver.get(game_url)
        ...
        driver.find_element(By.ID, "pauseBtn").click()
        time.sleep(0.2)
        state = driver.execute_script("return {running, paused}")
    ...
    def test_restart_resets_state(self, driver, game_url):
        ...
        driver.find_element(By.ID, "startBtn").click()
        time.sleep(0.2)
        driver.execute_script("score = 9; player.x = 260; player.y = 280;")
        driver.find_element(By.ID, "restartBtn").click()
        time.sleep(0.2)
        state = driver.execute_script("return {running, paused, score, x: player.x, y: player.y}")
        assert state["running"] == 0
        assert state["paused"] == 0
